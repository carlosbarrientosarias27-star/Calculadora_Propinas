...
def menu_interactivo():
    ...
    Incluye un bucle para repetir el proceso y una opción para salir.
    """
    while True:
        print("\n--- 🧮 CALCULADORA DE PROPINAS INTERACTIVA ---")
        print("Escriba 'salir' en cualquier momento para finalizar.") # Nuevo indicador de salida
        
        try:
            entrada_cuenta = input("Ingrese el total de la cuenta: ").lower()
            if entrada_cuenta == 'salir':
                print("¡Gracias por usar la calculadora! 👋")
                break # Rompe el bucle para salir
            
            cuenta = float(entrada_cuenta)
